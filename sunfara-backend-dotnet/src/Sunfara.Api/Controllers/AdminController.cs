...
public sealed class AdminController(FirestoreCatalogStore store) : ControllerBase
{
    ...
    [HttpPost("withdrawals/{id}/approve")] public async Task<IActionResult> ApproveWithdrawal(string id) => await store.UpdateAsync("withdrawals",id,new(){["status"]="approved",["approvedAt"]=DateTime.UtcNow}) ? Ok() : NotFound();
